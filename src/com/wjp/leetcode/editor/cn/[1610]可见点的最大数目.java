...
// 
// Related Topics 几何 数组 数学 排序 滑动窗口 👍 55 👎 0
    ...
    class Solution {
        public int visiblePoints(List<List<Integer>> points, int angle, List<Integer> location) {
            ...
            for (List<Integer> p : points) {
                ...
                // atan2(y,x) 返回坐标为 [-pi,pi]之间的角度
                list.add(Math.atan2(b - y, a - x) + pi);
            }
            // 想要在相距差值小于 angel的范围内，寻找可以看到的点的个数
            // 滑动一个窗口，使其范围内的point个数最大
            // 由于返回的角度范围是 [-pi,pi],但假设该窗口起点靠近尾部，其终点绕过其起点
            ...
            while (j < 2 * n) {
                // 符合条件下长度 j - i
                while (j < 2 * n && list.get(j) - list.get(i) <= t) {
                    j++;
                }
                int count = j - i;
