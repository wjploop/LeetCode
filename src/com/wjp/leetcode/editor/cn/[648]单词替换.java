...
//
// 
// Related Topics 字典树 数组 哈希表 字符串 👍 238 👎 0
    ...
    class Solution {
        public String replaceWords(List<String> dictionary, String sentence) {
            ...
            for (String word : dictionary) {
                ...
                cur.children.put('#', new Node());
            }
//            print(tree);
            String[] words = sentence.split(" ");
            String[] resWords = new String[words.length];
            for (int i = 0; i < words.length; i++) {
                resWords[i] = prefix(words[i], tree);
            }
            return String.join(" ", resWords);
        }

        private String prefix(String word, Node cur) {
        ...
        private void print(Node tree) {
            ...
            while (!queue.isEmpty()) {
                ...
                System.out.println(String.join(",", node.children.keySet().stream().map(character -> character.toString()).collect(Collectors.toList())));
